  ...
  <div>
    ...
    <section class="m-6 sm:m-16 z-50" data-aos="fade-up" data-aos-duration="1000">
      ...
      <div class="bg-customBGCards rounded-xl shadow-xl p-6 mt-10 sm:w-full mx-auto school ">
        <span class="text-3xl  text-customDarkPurple px-5 py-3 rounded-2xl flex items-center gap-2 m-10"> <i
            class="fa-duotone fa-sparkles"></i>بيانات المدرسة </span>
        <div class="input_Div grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div class="flex flex-col input-group">

            <input autocomplete="off" required="" @input="validInputs" type="text" v-model="name" id="in1"
              class="outline-customDarkPurple input" />
            <label for="in1" class="user-label">اسم المدرسة<span class="text-red-600">*</span></label>
          </div>
          <div class="flex flex-col input-group">
            <input autocomplete="off" required="" type="text" id="in5" v-model="phone" @input="validInputs"
              class="input" />
            <label for="in5" class="user-label">رقم هاتف المدرسة
              <span class="text-red-600">*</span></label>
          </div>
          <div class="flex flex-col input-group">

            <input autocomplete="off" required="" type="text" id="in6" v-model="address" @input="validInputs"
              class="input" />
            <label for="in6" class="user-label">عنوان المدرسة
              <span class="text-red-600 ">*</span></label>
            <span class="text-sm text-red-600 mx-2 mt-1">العنوان بالتفصيل - المحافظة : 270 شارع بورسعيد القاهره</span>
          </div>
          <div class="flex flex-col">
            <div class="custom-select relative">
              <select v-model="country" @change="validInputs" id="in2"
        ...
        <div class="flex flex-col w-full sm:w-auto mb-4 sm:mb-0 mt-3">
          ...
          <div class="relative" @drop="handleFileDrop" @dragover.prevent>
            <input type="file" id="in8" class="hidden " @change="handleFile" />
            <label for="in8" class=" cursor-pointer Filelabel " v-if="!file">
              <span>اختر ملف</span>
              او
              <span class="text-customDarkPurple font-bold">قم بسحب الملف هنا</span>
            </label>
            <div v-if="file">
              <i class="fa-thin fa-circle-xmark text-red-700 text-2xl p-4 cursor-pointer" @click="DeleteFile"></i>
              <img :src="imageURL" alt="error" class="w-60 h-60 m-3 rounded-full" />
            </div>
      ...
      <div class="bg-customBGCards rounded-xl shadow-xl p-6 mt-10 sm:w-full mx-auto">
        <span class="text-3xl  text-customDarkPurple px-5 py-3 rounded-2xl flex items-center gap-2 m-10"> <i
            class="fa-duotone fa-sparkles"></i>بيانات المدير </span>
        <div class="input_Div grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div class="flex flex-col input-group">
            <input autocomplete="off" required="" @input="validInputs" v-model="Manger_Name" type="text" id="in10"
              class="outline-customDarkPurple input" />
            <label for="in10" class="user-label">اسم المدير
              <span class="text-red-600">*</span></label>
          </div>
          <div class="flex flex-col input-group">
            <input autocomplete="off" required="" type="text" id="in11" v-model="Manger_Phone" @input="validInputs"
              :class="{ error: checkPhone }" class="input" />
            <label for="in11" class="user-label">رقم هاتف المدير
              <span class="text-red-600">*</span></label>
          ...
          <div class="flex flex-col input-group relative">
            <input autocomplete="off" required="" type="text" id="in12" v-model="Manger_Email" @input="validInputs"
              :class="{ error: checkEmail || checkEmailCaps }" class="input" />
            <label for="in12" class="user-label">البريد الالكتروني للمدير
              <span class="text-red-600">*</span></label>
            <div class="flex flex-col">
          ...
          <div class="flex flex-col input-group">
            <input autocomplete="off" required="" type="text" id="in14" v-model="Manger_Address" @input="validInputs"
              class="input" />
      ...
      <div class="m-5 mt-10 flex items-center justify-end sm:gap-5">
        <button class="w-60" @click="add_New_School" :class="{ disabledBtn: close }" :disabled="close">
          حفظ
        </button>
        <button class="w-48" @click="this.$router.replace('/Admin/Schools')">
          الغاء
        </button>
      </div>
    </section>
