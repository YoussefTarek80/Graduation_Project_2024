    ...
    <div class="flex w-full items-center justify-between gap-5 ">
      <div class="w-full relative">
        ...
        >
          ...
          <option v-for="stage in GetStagesStud" :key="stage.id" :value="stage.id">{{ stage.stage_name }}</option>
        </select>
      ...
      <div class="w-full relative">
        ...
        >
          ...
          <option v-for="level in GetLevelsStud" :key="level.id" :value="level.id">{{ level.grade_name }}</option>
        </select>
        <i class="fa-solid fa-arrow-down-big-small absolute left-5 top-2 text-2xl"></i>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters(["GetStagesStud","GetLevelsStud","GetAllTerms"])
  },
  async created() {
    console.log(this.$route.params.index)
    const studID=this.$route.params.index;
    await this.FetchStageStudent(studID);
  },
  methods: {
    ...mapActions(["FetchStageStudent", "FetchLevelStudent", "GetTerms"]),
    async selectStage(event) {
      this.stageID = event.target.value;
      await this.FetchLevelStudent({studentId:this.$route.params.index,stageId:this.stageID });
      this.select = false;
      this.$emit('Retrived-false', this.RetrivedDone);
    },
    async selectlevel(event) {
      this.levelID = event.target.value;
      const selectedLevel = this.GetLevelsStud.find(level => level.id == this.levelID);
